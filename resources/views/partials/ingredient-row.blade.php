...
<div class="ingredient-row grid grid-cols-12 gap-3 items-start">
    <div class="col-span-12 sm:col-span-6">
        ...
        <select name="ingredients[{{ $index }}][product_id]"
                onchange="toggleNewProductFields(this)"
                {{ $required ? 'required' : '' }}
                class="ingredient-product-select w-full rounded-lg border border-gray-300 px-3 py-2 focus:border-blue-500 focus:ring-blue-500">
            <option value="">— Выберите продукт —</option>
            <option value="new" @selected($productVal === 'new')>🆕 Создать новый продукт</option>
            @foreach ($products as $product)
                <option value="{{ $product->id }}"
                    @selected(!empty($productVal) && (string) $productVal === (string) $product->id)>
    ...
    <div class="col-span-12 new-product-fields {{ $isNew || $productVal === 'new' ? '' : 'hidden' }}">
